...
const {
  compose, concat, curry, forEach, map, reduce,
} = require('./support');
...
const runSolutions = compose(
  defineSpecs('solution'),
  readExercises,
);

const runExercises = compose(
  defineSpecs('exercise'),
  readExercises,
);
...
Available options:
  ...
  const spec = curry(
    (runner, chapter) => `const { describe } = require('mocha');
